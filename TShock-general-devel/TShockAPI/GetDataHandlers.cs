		...
		private static bool HandleTile(GetDataHandlerArgs args)
		{
			...
			try
			{
				...
				if (action == EditAction.KillTile && !Main.tileCut[tile.type] && !breakableTiles.Contains(tile.type))
				{
					...
					if (Main.tileAxe[tile.type] && ((args.Player.TPlayer.mount.Type != 8 && selectedItem.axe == 0) && args.Player.RecentFuse == 0))
					{
						args.Player.SendTileSquare(tileX, tileY, 4);
						return true;
					}
					// If the tile is a hammer tile and they aren't selecting a hammer, they're hacking.
					else if (Main.tileHammer[tile.type] && ((args.Player.TPlayer.mount.Type != 8 && selectedItem.hammer == 0) && args.Player.RecentFuse == 0))
					{
						args.Player.SendTileSquare(tileX, tileY, 4);
						return true;
					}
					// If the tile is a pickaxe tile and they aren't selecting a pickaxe, they're hacking.
					else if ((!Main.tileAxe[tile.type] && !Main.tileHammer[tile.type]) && tile.wall == 0 && ((args.TPlayer.mount.Type != 8 && selectedItem.pick == 0) && args.Player.RecentFuse == 0))
					{
						args.Player.SendTileSquare(tileX, tileY, 4);
						return true;
					}
				...
				else if (action == EditAction.KillWall)
				{
					...
					if (selectedItem.hammer == 0 && args.Player.RecentFuse == 0 && selectedItem.createWall == 0)
					{
						args.Player.SendTileSquare(tileX, tileY, 1);
						return true;
					}
				...
				if (TShock.CheckRangePermission(args.Player, tileX, tileY))
				{
					...
					if (action == EditAction.KillTile || action == EditAction.KillWall && args.Player.RecentFuse == 0)
					{
						return false;
					}
