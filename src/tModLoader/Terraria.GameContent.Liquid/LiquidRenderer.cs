...
namespace Terraria.GameContent.Liquid
{
	public class LiquidRenderer
	{
		...
		private LiquidRenderer.LiquidCache[] _cache = new LiquidRenderer.LiquidCache[41617];
		private LiquidRenderer.LiquidDrawCache[] _drawCache = new LiquidRenderer.LiquidDrawCache[40001];
						...
						for (int num18 = 2; num18 < rectangle.Width - 2; num18++)
						{
							...
							for (int num19 = 2; num19 < rectangle.Height - 2; num19++)
							{
								if (ptr2->HasVisibleLiquid)
								{
									...
									ptr6->A = b;
									LiquidRenderer.LiquidCache* ptr8 = ptr2 - 1;
									if (num19 != 2 && !ptr8->HasVisibleLiquid && !ptr8->IsSolid && !ptr8->IsHalfBrick)
									{
										*(ptr6 - 200) = *ptr6;
									}
								}
					...
					for (int num26 = rectangle.Y; num26 < rectangle.Y + rectangle.Height; num26++)
					{
						if (ptr2->VisibleType == 1 && ptr2->HasVisibleLiquid && Dust.lavaBubbles < 200)
						{
							...
							if (this._random.Next(350) == 0)
							{
								...
								Dust expr_1184_cp_0 = Main.dust[num27];
								expr_1184_cp_0.velocity.X = expr_1184_cp_0.velocity.X * 2f;
								Dust expr_11A2_cp_0 = Main.dust[num27];
								expr_11A2_cp_0.velocity.Y = expr_11A2_cp_0.velocity.Y - (float)this._random.Next(1, 7) * 0.1f;
								if (this._random.Next(10) == 0)
								{
									Dust expr_11DE_cp_0 = Main.dust[num27];
									expr_11DE_cp_0.velocity.Y = expr_11DE_cp_0.velocity.Y * (float)this._random.Next(2, 5);
								}
