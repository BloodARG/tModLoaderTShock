		...
		public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
		{
			...
			for (int k = num; k < num2; k++)
			{
				if (this._balloons[k].Active)
				{
					...
					Vector2 value3 = new Vector2(1f / this._balloons[k].Depth, 0.9f / this._balloons[k].Depth);
					Vector2 vector = this._balloons[k].Position;
					...
					{
						spriteBatch.Draw(this._balloons[k].Texture, vector, new Rectangle?(this._balloons[k].GetSourceRectangle()), value2 * this._opacity, 0f, Vector2.Zero, value3.X * 2f, SpriteEffects.None, 0f);
					}
				}
			}
		}
