...
namespace Terraria.GameContent.UI
{
	public class EmoteBubble
	{
		...
		internal static int[] CountNPCs = new int[580];
		...
		private void Draw(SpriteBatch sb)
		{
			...
			Rectangle value = texture2D.Frame(8, 33, flag ? 0 : 1, 0);
			Vector2 origin = new Vector2((float)(value.Width / 2), (float)value.Height);
			...
			sb.Draw(texture2D, vector, new Rectangle?(value), Color.White, 0f, origin, 1f, spriteEffects, 0f);
			if (flag)
			{
				return;
			}
			...
			if (this.emote == -1)
			{
				...
				sb.Draw(texture2D, vector + new Vector2((float)(spriteEffects.HasFlag(SpriteEffects.FlipHorizontally) ? 1 : -1), (float)(-(float)value.Height + 3)), null, Color.White, 0f, new Vector2((float)(texture2D.Width / 2), 0f), num, spriteEffects, 0f);
			}
		}
