...
namespace Terraria
{
	public class MessageBuffer
	{
		...
		public void GetData(int start, int length, out int messageType)
		{
			...
			{
                ServerApi.LogWriter.ServerWriteLine(string.Format("getdata: slot {0}: msg id {1} on client state {2}", whoAmI, b, Netplay.Clients[this.whoAmI].State), TraceLevel.Warning);
				...
				case 4:
					{
						...
						if (flag)
						{
                            if (!ServerApi.Hooks.InvokeNetNameCollision(num7, player2.name))
                            {
                                NetMessage.SendData(2, this.whoAmI, -1, string.Concat(player2.name, " ", Lang.mp[5]), 0, 0f, 0f, 0f, 0, 0, 0);
                            }
                            return;
                        }
